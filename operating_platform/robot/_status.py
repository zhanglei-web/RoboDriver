...
class MachineInformationPost:
    ...
    def update_connection_status(self, device_type=None, device_name=None, new_status=None, set_all=None):
        """更新设备连接状态（线程安全）"""
        with self._lock:
            ...
            if set_all is not None and device_type is not None:
                ...
                devices = self._machine_info_dict["specifications"].get(device_type, {}).get("information", [])
            ...
            # 单个设备设置
                ...
                raise ValueError("参数组合错误：请提供 (device_type+device_name+new_status) 或 (device_type+set_all+new_status)")
...
if __name__ == "__main__":
    ...
    # 更新状态（非阻塞）
    machine_post.update_connection_status(device_type="camera", set_all=True, new_status=True)
    machine_post.update_connection_status(device_type="piper", set_all=True, new_status=True)
