...
from operating_platform.robot.robots.galaxea_v1 import GALAXEARobotConfig
from operating_platform.robot.robots.galaxea_v1 import GALAXEARobotStatus
...
def recv_image_server():
    ...
    while running_recv_image_server:
        try:
            ...
            logger.warning(f"GALAXEA Image Received Timeout")
            continue
...
def recv_joint_server():
    ...
    while running_recv_joint_server:
        try:
            ...
            logger.warning(f"GALAXEA Joint Received Timeout")
            continue
        except Exception as e:
...
class GALAXEAManipulator:
    def __init__(self, config: GALAXEARobotConfig):
        ...
        self.status = GALAXEARobotStatus()
        self.use_videos = self.config.use_videos
        self.microphones = self.config.microphones # galaxea没有audio

        self.leader_arms = {}
        self.leader_arms['leader_arms'] = self.config.leader_arms["main"]

        self.follower_arms = {}
        self.follower_arms['follower_arms'] = self.config.follower_arms["main"]
    ...
    def teleop_step(
        ...
        for name in self.cameras:
            ...
            images[name] = torch.from_numpy(np.copy(images[name]))
    ...
    def send_action(self, action: dict[str, Any]):
        ...
        galaxea_zmq_send(f"action_joint", goal_joint_numpy, wait_time_s=0.01)
